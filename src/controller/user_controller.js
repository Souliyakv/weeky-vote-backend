    ...
    try {
        ...
        con.query(CHECKEMAIL,[email],async(err,result)=>{
            ...
            if(result === undefined || result.length > 0){
                ...
                const values = [[uid,fname,lname,email,getPassword,position,image]];
                con.query(REGISTER,[values],(err,resDate)=>{
                    if(err) throw err;
                    if(resDate.affectedRows == 0)return res.json({msg:"ບໍ່ສາມາດສະໝັກສະມາຊິກໄດ້"});
                    const token = generateToken(uid);
                    return res.status(201).json({msg:"ສ້າງສະມາຊິກສຳເຫຼັດ",
                token:token})
                })
            }
        })
...
export const Login_Controller = (req,res)=>{
    ...
    try {
        ...
        con.query(CHECKEMAIL,[email],async(err,result)=>{
            if(err) throw err;
            if(result === undefined || result.length <=0)return res.json({msg:"ບໍ່ມີບັນຊີຜູ້ໃຊ້ນີ້"});
            ...
            return res.status(201).json({msg:"ເຂົ້າສູ່ລະບົບສຳເຫຼັດ",
        token:token});
        });
